package com.example.imgedit.editLibrary.core;
...
import com.example.imgedit.editLibrary.core.clip.IMGClip;
import com.example.imgedit.editLibrary.core.clip.IMGClipWindow;
...
public class IMGImage {
    ...
    private List<IMGPath> mShades = new ArrayList<>();
    ...
    public boolean isShadeEmpty() {
        return mShades.isEmpty();
    }

    public void undoShade() {
        if (!mShades.isEmpty()) {
            mShades.remove(mShades.size() - 1);
        }
    }
...
//                    mClipWin
    ...
    public void onDrawShades(Canvas canvas) {
        if (!isShadeEmpty()) {
            ...
            for (IMGPath path : mShades) {
                path.onDrawShade(canvas, mPaint);
            }
            canvas.restore();
        }
    }
