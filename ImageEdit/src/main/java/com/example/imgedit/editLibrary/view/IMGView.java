package com.example.imgedit.editLibrary.view;
...
import com.example.imgedit.editLibrary.core.anim.IMGHomingAnimator;
...
public class IMGView extends FrameLayout implements Runnable, ScaleGestureDetector.OnScaleGestureListener,
...

    {
        ...
        mShadePaint.setStrokeWidth(IMGPath.BASE_DOODLE_WIDTH);
        mShadePaint.setColor(Color.RED);
        mShadePaint.setPathEffect(new PathEffect());
    ...
    private static class Pen extends IMGPath {
        ...
        void lineTo(IMGImage mImage, float x, float y) {
            if (mImage.getMode() == IMGMode.SHADE) {
                ...
                if (downX < upX) {
                    if (downY < upY) {
                        this.path.addRect(downX, downY, upX, upY, Path.Direction.CCW);
                    } else {
                        this.path.addRect(downX, upY, upX, downY, Path.Direction.CCW);
                    }
                } else {
                    if (downY < upY) {
                        this.path.addRect(upX, downY, downX, upY, Path.Direction.CCW);
                    } else {
